...
const LensesPage: FC = () => {
    ...
    const updateLenses = () => {
        let min_price_numeric: number = parseInt(searchLensMinPrice)
        if (isNaN(min_price_numeric)) min_price_numeric = 0
        let max_price_numeric: number = parseInt(searchLensMaxPrice)
        if (isNaN(max_price_numeric)) max_price_numeric = 1000000
        getLensesByName(searchLensName, min_price_numeric, max_price_numeric).then((response) => {
    ...
    return (
        <>
            ...
            <div className='d-flex flex-column'>
                <div className='d-flex w-100 search_and_basket'>
                    ...
                    <img src='/src/assets/img/empty_basket.jpeg' className='basket_img'></img>
                </div>

                <div className='d-flex flex-wrap gap-5 ms-4 mt-5 w-100'>
                    {lenses.map((lens) => {
                        return (
                            <LensCard   
                                id={lens.lens_id}
